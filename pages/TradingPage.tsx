        ...
        {state.tradingBlocks.map((block, blockIdx) => {
          ...
          return (
            <motion.section
              ...
              viewport={{ once: true }}
              className="bg-white rounded-[3rem] border border-slate-200 shadow-xl overflow-hidden relative"
            >
              <div className="absolute top-0 right-0 w-96 h-96 bg-blue-50 rounded-full blur-[100px] -mr-48 -mt-48 opacity-30 z-0" />

              <div className="relative z-10">
                {/* Block Header */}
                <header className="bg-slate-50/50 p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
                  <div className="flex items-center gap-4 w-full md:w-auto">
                    <div className="bg-blue-600 text-white p-3 rounded-2xl shadow-lg shadow-blue-200">
                      <LayoutGrid size={24} />
                    </div>
                    <input
                      className="bg-transparent border-none text-2xl font-black text-slate-900 focus:ring-0 w-full md:w-64 placeholder-slate-300"
                      value={block.title}
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {block.priceRanges.map((row, rowIdx) => (
                            <tr key={rowIdx} className="hover:bg-white transition-colors">
                              <td className="px-4 py-3 font-bold text-slate-800 whitespace-nowrap">{row.range}</td>
                              ...
                              <td className="px-2 py-2">
                                <input type="number" className="w-full bg-white border border-slate-100 rounded-lg px-2 py-1.5 focus:ring-2 focus:ring-blue-400 outline-none text-right font-black text-blue-600" value={row.pnl || ''} onChange={(e) => handlePriceRangeChange(blockIdx, rowIdx, 'pnl', Number(e.target.value))} />
                              </td>
                            </tr>
                          ))}
                        </tbody>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between px-2">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-[9px] font-black text-slate-400 uppercase">Unrealized:</span>
                          ...
                          {block.activities.map((row, rowIdx) => (
                            <tr key={rowIdx} className="hover:bg-white transition-colors">
                              <td className="px-2 p-1">
                                <input className="w-full bg-white border border-slate-100 rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-indigo-400 outline-none text-left font-medium" value={row.activity} onChange={(e) => handleActivityChange(blockIdx, rowIdx, 'activity', e.target.value)} placeholder="Note..." />
                              </td>
                              <td className="px-1 py-1">
                                <input type="number" className="w-full bg-white border border-slate-100 rounded-lg px-2 py-1.5 focus:ring-2 focus:ring-indigo-400 outline-none text-center font-bold" value={row.share || ''} onChange={(e) => handleActivityChange(blockIdx, rowIdx, 'share', Number(e.target.value))} />
                              </td>
                              <td className="px-1 py-1">
                                <input type="number" className="w-full bg-white border border-slate-100 rounded-lg px-2 py-1.5 focus:ring-2 focus:ring-indigo-400 outline-none text-center font-bold" value={row.cost || ''} onChange={(e) => handleActivityChange(blockIdx, rowIdx, 'cost', Number(e.target.value))} />
                              </td>
                              <td className="px-1 py-1">
                                <input type="number" className="w-full bg-white border border-slate-100 rounded-lg px-2 py-1.5 focus:ring-2 focus:ring-indigo-400 outline-none text-center font-bold" value={row.sold || ''} onChange={(e) => handleActivityChange(blockIdx, rowIdx, 'sold', Number(e.target.value))} />
                              </td>
                            </tr>
                          ))}
                        </tbody>
